...
@endif
<form action="{{route('posts.store')}}" class="card card-body bg-secondary" method="POST">
        @csrf
        @method('POST')
        ...
        <select class="form-select" name="access" id="access" required>
            <option value="true" value="{{ old('access') }}">Private</option>
            <option value="false" value="{{ old('access') }}">Public</option>
        </select>
        </div>
